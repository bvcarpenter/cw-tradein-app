 ...
 * Reuses the AUTH_KV binding (same KV namespace as auth)
 * Session keys are prefixed with "cwti_" so there's no collision with auth keys
 *
...
export async function onRequestPost({ request, env }) {
  const kv = env.AUTH_KV;
  if (!kv) return json({ error: 'AUTH_KV not bound' }, 503);
